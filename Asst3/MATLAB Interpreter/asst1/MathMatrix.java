...
public class MathMatrix extends MathObject {
	...
	public MathMatrix add(MathScalar scal) {
		int size = this.matrixArray.length;
		double[][] a = new double[size][size];
		MathMatrix m = new MathMatrix(a);
		for (int i=0; i<size; i++)
	...
	public int getCols(int i) {
		return this.matrixArray[i].length;
	}
	/**
	...
	public MathMatrix add(MathMatrix mat) throws MathException {
		if((this.getColumns()!= mat.getColumns())||(this.getRows()!=mat.getRows())){ throw new MathException();}
		double[][] a = new double[this.getRows()][this.getColumns()];
		
		for (int i=0; i<getRows(); i++) {
			for (int j=0; j<mat.getColumns(); j++) {
					a[i][j]= this.matrixArray[i][j]+mat.matrixArray[i][j];
			}
	...
	public double returnIndex(int i, int j) {
